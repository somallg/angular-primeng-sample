...
module.exports = gulp => () => {
  ...
  const currentVersion = require('semver').parse(
    require('../../package.json').version
  );
  const baseHead = shelljs
    .exec(
      `git ls-remote --heads origin ${currentVersion.major}.${
  ...
  console.log(
    `Examining ${commitsByLine.length} commits between HEAD and ${baseBranch}`
  );

  if (commitsByLine.length == 0) {
    console.log(
      `There are zero new commits between this HEAD and ${baseBranch}`
    );
  }
